...
class JsHttpRequest
{
    ...
    function JsHttpRequest($enc)
    {
        ...
        if (preg_match('/^(.*)(?:&|^)JsHttpRequest=(?:(\d+)-)?([^&]+)((?:&|$).*)$/s', @$_SERVER['QUERY_STRING'], $m)) {
            ...
            $this->_unicodeConvMethod = null;//function_exists('mb_convert_encoding')? 'mb' : (function_exists('iconv')? 'iconv' : null);
    ...
    {
        // little speedup by using array_flip($this->_encTables) and later hash access in the constructor.
        ...
        if($toEnc == 'utf-8')
        {
            /* UTF-8 conversion rules were taken from http://www.cl.cam.ac.uk/~mgk25/unicode.html */
            
            if($code < 0x800)
            {
                return chr(0xC0 + ($code >> 6)).
                       chr(0x80 + ($code & 0x3F));
            }else // if ($code <= 0xFFFF) -- it is already so for UCS2-BE
            {
